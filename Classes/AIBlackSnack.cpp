	...
	if (targetCharacter)
	{
		...
		else
		{
			...
			if (!targetBuffHandler->exist(
				ToolFunction::WStr2UTF8(L"ViewSizeBuff_÷öÃğ_ATTR_Bad_1_10_-3")))
			{
				//Èç¹ûµ±Ç°Ä¿±êÃ»ÓĞÖÂÃ¤
				//ÒÑÒ÷³ª
				if (characterPtr->getChantCount() >= 1)
				{
					characterPtr->runSkill(
						ToolFunction::WStr2UTF8(L"BuffCast_÷öÃğ_0_0_ViewSizeBuff_÷öÃğ_ATTR_Bad_1_10_-3")
					);
					HudMessageBox::getInstance()->addMessage(
						targetCharacter->getName() +
						ToolFunction::WStr2UTF8(L"ÖĞÁË÷öÃğbuff")
					);
				}
