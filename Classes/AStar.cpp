...
std::vector<cocos2d::Point> AStar::findPath_v2()
{
	//g表示从起点到自己的代价，h表示从自己到终点的代价
	//把起始点放入到开启列表中
	AStarPoint *start_point = new AStarPoint();
	...
	while (0 != opened_list.size())
	{
		...
		for (int i = top_left_x; i <= bottom_right_x; ++i)
		{
			for (int j = top_left_y; j <= bottom_right_y; ++j)
			{
				...
				{
					//非障碍物
					if (storey->isMoveAble_v2(cocos2d::Point(i, j))
						|| cocos2d::Point(i, j) == endPoint)
						//					if(storey->isMoveAble(storey->getTile(i,j)))
					{
						AStarPoint point;
...
std::vector<cocos2d::Point> AStar::findPath_v3()
{
	//g表示从起点到自己的代价，h表示从自己到终点的代价
	//把起始点放入到开启列表中
	AStarPoint *start_point = new AStarPoint();
	...
	while (0 != opened_list.size())
	{
		...
		for (int i = top_left_x; i <= bottom_right_x; ++i)
		{
			for (int j = top_left_y; j <= bottom_right_y; ++j)
			{
				...
				{
					//非障碍物
					if (storey->isMoveAble_v3(cocos2d::Point(i, j))
						|| cocos2d::Point(i, j) == endPoint)
						//					if(storey->isMoveAble(storey->getTile(i,j)))
					{
						AStarPoint point;
...
cocos2d::Point AStar::nextStep_v3()
{
	std::vector<cocos2d::Point> path = findPath_v3();
